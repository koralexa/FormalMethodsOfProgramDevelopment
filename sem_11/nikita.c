...
// ghost - код, используемый исключительно для верификации
  ...
  while (i + j < len1 + len2) {
    ...
    if (i == len1) {
      ...
      /*@ ghost
        ...
        @  @ loop invariant \forall integer x; count{Before}(result, 0, q, x) == count(result, 0, q, x);
      ...
      */
      /*@
        @ assert \forall integer x; count{Before}(result, 0, i + j - 1, x) == count(result, 0, i + j - 1, x);
      */
      /*@
